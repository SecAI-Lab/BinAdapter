...
class Preprocess:
    def __init__(self, prefix, inp_dir, src_file, trg_file, bpe_file):
        self.prefix = prefix
    ...
    def tokenize_names(self, codes=None, names=None):
        ...
        if names and codes:
            ...
            for name, code in zip(names, codes):
                p = re.compile('[^a-zA-Z]')
                p = p.sub(' ', name).strip()
                camel_letters = re.findall(
                    r'[A-Za-z](?:[a-z]+|[A-Z]*(?=[A-Z]|$))', p)
    ...
    def apply_bpe(self, num_operations, voc_file):
        print('Applying BPE....')
        bpe_learn_cmd = f"subword-nmt learn-bpe -s {num_operations} < {self.src_file} > {voc_file}"
...
if __name__ == '__main__':
    data_prefix = 'case3'
    p = Preprocess(prefix=data_prefix,
                   inp_dir='./ghidra/store/case3/',
